...
export function MerchantPendingPage() {
  ...
  const handleRefresh = async () => {
    ...
    // If approved, the auth context will update and routing will redirect
    if (result.profile?.merchant_status === "approved") {
      navigate("/merchant/orders");
    }
  };

  const isRejected = profile?.merchant_status === "rejected";
            ...
            <div className="flex items-center gap-3">
              <div
                className={`w-3 h-3 rounded-full ${
                  isRejected ? "bg-destructive" : "bg-yellow-500 animate-pulse"
                }`}
              />
              <div>
